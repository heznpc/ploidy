...
AceMAD provides the theoretical foundation demonstrating \emph{that} asymmetry works. Ploidy serves as empirical validation and system-level implementation: answering the architectural questions of \emph{what form} of context asymmetry to inject (the Context Asymmetry Spectrum), \emph{how} to deliver it (injection mode), and \emph{how many} sessions are needed to separate structural bias from stochastic noise (ploidy level).
...
Effort is not monotonically beneficial. At max effort, both methods produce the worst recall (Single~3.5/5, Ploidy~3.0/5)---the model appears to overthink, generating over-elaborate analyses that diverge from ground-truth findings. Effort=low matches effort=high on recall (4.5/5) at roughly half the compute time. Ploidy outperforms Single on F1 only at effort=high, suggesting the effort~$\times$~method interaction is non-trivial: context-asymmetric debate is most effective at intermediate reasoning depth.
...
Self-Consistency (5-vote) achieves comparable recall to Ploidy on long-context tasks, raising the question of whether the structured protocol adds value beyond majority voting.

Two distinctions apply. First, Self-Consistency's output is a deduplicated list with vote counts---it answers \emph{what} was found but not \emph{why} sessions disagreed. Ploidy's convergence output provides typed semantic actions and context attribution, enabling the user to understand whether a disagreement is context-driven (the Deep session's history caused anchoring) or stochastic (random variance between runs). This interpretability is not available from majority voting.

Second, Self-Consistency is vulnerable to correlated bias. When all 5 sessions receive the same context (including the same memory-style injection), they share the same anchoring priors. Our injection sweep shows that memory-style context degrades Single recall by 20\%---and Self-Consistency, which runs 5 copies of Single, would inherit this degradation identically across all copies. Ploidy's Fresh sessions, having never seen the memory-formatted context, are immune to this correlated failure mode.
...
\begin{enumerate}[label=\arabic*., leftmargin=1.5em]
  ...
  \item \textbf{Effort level is not monotonically beneficial.} Max effort produces the worst recall for both methods, consistent with overthinking effects. The effort~$\times$~method interaction is non-trivial: Ploidy's F1 advantage appears only at intermediate reasoning depth.
\end{enumerate}
